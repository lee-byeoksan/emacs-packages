...
def serve(runtime, command, identity=None, on_state=None):
    ...
    def state():
        value = dict(daemon_pid=os.getpid(), child_pid=child,
    ...
    try:
        while not stop:
            ...
            if control and control in ready:
                raw, address = control.recvfrom(2048)
                try:
                    request = json.loads(raw)
                    if request.get('id') != identity:
                        raise ValueError('Owner mismatch')
                    action = request.get('action')
                    if action not in ('status', 'stop'):
                        raise ValueError('Unknown control action')
                    response = dict(id=identity, state='running',
                                    attached_clients=int(client is not None), recorder_pid=child)
                    if action == 'stop':
                        stop = True
                    if address:
        ...
        if control:
            control.close()
            (runtime / 'control').unlink(missing_ok=True)
        state()
        os.close(lock)


def attach(runtime, on_ready=None):
    runtime = private_runtime(runtime)
                ...
                if not data:
                    ...
                    for kind, payload in packets(incoming):
                        if kind == b'O':
                            ...
                            if on_ready:
                                on_ready(json.loads(payload))
                            if not json.loads(payload)['replay_complete']:
                                output.extend(b'\r\n[EAM PTY: screen restoration unavailable (1MiB replay limit); C-c a h opens conversation history]\r\n')
            if 0 in ready:
